 ...
 */
export const nearestNeighborTSP = (
  waypoints,
  startIndex = 0,
  distanceMatrix = null
) => {
  ...
  while (improved) {
    ...
    for (let i = 0; i < n - 2; i++) {
      for (let j = i + 2; j < n; j++) {
        ...
        const d2 =
          j + 1 < n ? distanceMatrix[currentRoute[j]][currentRoute[j + 1]] : 0
...
export const splitRouteByBattery = (orderedWaypoints, droneSpec, homePoint) => {
  ...
  for (let i = 0; i < orderedWaypoints.length; i++) {
    ...
    if (
      totalIfVisit > maxFlightDistance &&
      currentFlight.waypoints.length > 0
    ) {
      // 現在のフライトを保存
      currentFlight.returnDistance = getDistance(lastPoint, homePoint)
      currentFlight.totalDistance =
        accumulatedDistance + currentFlight.returnDistance
...
export const optimizeRoute = async (waypoints, options = {}) => {
  ...
  // 3. TSPで最適順序を計算
  let routeIndices = nearestNeighborTSP(
    waypoints,
    optimalStart.index,
    distanceMatrix
  )
  ...
  if (autoSplit) {
    ...
    const returnDist = getDistance(
      orderedWaypoints[orderedWaypoints.length - 1],
      effectiveHomePoint
    )
    flights = [
      {
  ...
  // 9. サマリー計算
  const totalDistance = processedFlights.reduce(
    (sum, f) => sum + f.totalDistance,
    0
  )
  const totalTime = processedFlights.reduce(
    (sum, f) => sum + f.estimatedTime,
    0
  )
