...
import {
  getPolygonCenter,
  calculatePolygonArea as getPolygonArea,
} from './waypointGenerator'
...
export const checkAllWaypointsRestrictions = (waypoints) => {
  ...
  const batchResults = checkWaypointsCollisionBatch(
    waypointsForCheck,
    spatialIndex
  )
...
export const checkAllWaypointsDID = async (waypoints) => {
  ...
  const areaSummaries = Array.from(checkedAreas.entries()).map(
    ([area, indices]) => ({
      area,
...
export const checkFlightPathCollision = (waypoints) => {
  ...
  if (import.meta.env.DEV) {
    console.log(
      '[PathCollision] Waypoint groups:',
      Array.from(waypointsByPolygon.entries()).map(
        ([id, wps]) =>
 ...
 * ローカルリスク判定
 */
export const analyzeFlightPlanLocal = async (
  polygons,
  waypoints,
  options = {}
) => {
  ...
  // 2. 最寄り空港
  const nearestAirport = center
    ? findNearestAirport(center.lat, center.lng)
    : null

  // 3. DIDチェック
  const waypointDIDCheck =
    waypoints.length > 0 ? await checkAllWaypointsDID(waypoints) : null

  const didInfo = waypointDIDCheck?.hasDIDWaypoints
    ? {
        isDID: true,
        description: 'DID内のWPがあります',
  ...
  if (restrictions.some((r) => r.type === 'airport'))
    requiredPermissions.push('空港周辺飛行許可')
  ...
  // UTMチェック
  const utmCheck = center
    ? checkUTMConflicts({ center })
    : { checked: false, message: '位置情報なし', conflicts: [] }
...
export const checkWaypointsRestrictionSurfaces = async (waypoints) => {
  ...
  try {
    ...
    console.warn(
      '[RiskService] checkWaypointsRestrictionSurfaces error:',
      error
    )
    return new Map()
  }
 ...
 * @returns {Object} 詳細な衝突検出結果
 */
export const getDetailedCollisionResultsWithRestrictionSurfaces = (
  waypoints,
  options = {}
) => {
  ...
  for (const [waypointId, rbushResult] of batchResults.entries()) {
    ...
    if (rsResult?.isInRestrictionSurface) {
      ...
      if (
        rbushResult.isColliding &&
        (rbushResult.collisionType === 'AIRPORT' ||
          rbushResult.collisionType === 'MILITARY')
      ) {
