...
export const USE_CASES = [
  ...
  },
  {
    ...
    tips: ['RTK対応機体推奨', 'GCP（基準点）を設置', 'オーバーラップ率80%以上'],
  },
  {
    id: 'agriculture',
    name: '農業（散布・監視）',
    ...
    tips: ['早朝または夕方が最適', '風速3m/s以下で実施', '散布ムラに注意'],
  },
  {
    id: 'delivery',
    name: '物流配送',
    ...
    tips: ['着陸地点の事前確認', '積載重量を確認', '天候予報を確認'],
  },
  {
    id: 'security',
    name: '警備・監視',
    ...
    tips: ['リアルタイム映像伝送', 'スペア機を準備', '群衆上空を避ける'],
  },
  {
    id: 'other',
    name: 'その他',
...
const evaluateRoute = async (route, useCase, options = {}) => {
  ...
  for (const wp of route.waypoints) {
    ...
    for (const airport of AIRPORT_ZONES) {
      ...
      if (
        dist < airport.radius &&
        !issues.find((i) => i.name === airport.name)
      ) {
        issues.push({
          type: 'airport',
          severity: 'error',
  ...
  if (
    distance > CAT3_DISTANCE_THRESHOLD ||
    issues.some((i) => i.type === 'did')
  ) {
  ...
  score -=
    issues.filter((i) => i.severity === 'warning').length * PENALTY_WARNING
...
export const generateRouteOptions = async (
  ...
  const avoidanceEval = await evaluateRoute(avoidanceRoute, useCase, {
    altitude,
  })
  avoidanceEval.name = 'DID回避ルート'
  avoidanceEval.description =
    avoidanceRoute.avoided.length > 0
  ...
  directEval.evaluation.cons =
    directEval.evaluation.issues.length > 0
      ? ['申請手続きが必要', 'DID/制限区域を通過']
      : []

  avoidanceEval.evaluation.pros =
    avoidanceRoute.avoided.length > 0
      ? ['申請手続きが簡素化', '法的リスクが低い']
      : ['直線経路で問題なし']
  avoidanceEval.evaluation.cons =
    avoidanceRoute.distance > directRoute.distance * AVOIDANCE_DISTANCE_FACTOR
  ...
  // 推奨ルートを決定
  const recommended =
    routes.find((r) => r.evaluation.recommendation === 'recommended') ||
    routes[0]
  ...
  return {
    ...
    summary: {
      ...
      distanceDiffPercent: Math.round(
        ((avoidanceRoute.distance - directRoute.distance) /
...
export const getUseCaseById = (id) => {
  return (
    USE_CASES.find((uc) => uc.id === id) ||
    USE_CASES.find((uc) => uc.id === 'other')
  )
}
