 ...
 */
const calculateDIDAvoidancePosition = (
  lat,
  lng,
  centroid,
  avoidanceDistance = 500
) => {
  ...
  const distanceToCenter = getDistanceMeters(
    lat,
    lng,
    centroid.lat,
    centroid.lng
...
export const analyzeWaypointGaps = (waypoints, didInfo = null) => {
  ...
  const recommendedWaypoints = waypoints.map((wp) => {
    const wpIndex =
      wp.index !== undefined ? wp.index : waypoints.indexOf(wp) + 1
    let newLat = wp.lat
    let newLng = wp.lng
    let modified = false
    const issues = []
    ...
    if (didData && shouldFlagDID) {
      ...
      if (didAvoidanceEnabled && didData.centroid) {
        ...
        const safe = calculateDIDAvoidancePosition(
          wp.lat,
          wp.lng,
          didData.centroid,
          didMargin
...
export const analyzePolygonGaps = (polygon) => {
  if (!polygon?.geometry?.coordinates?.[0])
    return { hasIssues: false, totalGaps: 0, recommendedPolygon: null }
  ...
  for (const [lng, lat] of coords) {
    ...
    for (const airport of AIRPORT_ZONES) {
      if (
        getDistanceMeters(lat, lng, airport.lat, airport.lng) < airport.radius
      ) {
        pointIssue = true
        break
      }
