...
export const checkSmallUASProhibitionLaw = (context) => {
  ...
  if (redZonesIn.length > 0) {
    results.items.push({
      ...
      status: 'error',
      statusText: '禁止区域内',
      description: redZonesIn
        .map((z) => `${z.name}（${Math.round(z.distance)}m）`)
        .join('、'),
...
export const checkLandManagerRules = (context) => {
  ...
  if (
    ...
    results.items.push({
      ...
      action: '河川事務所への確認推奨',
      notes: ['国管理河川: 国土交通省河川事務所', '県管理河川: 各都道府県'],
    })
    results.requiresPermission = true
  }

  // 道路上空チェック
  if (placeType.includes('road') || placeType.includes('highway')) {
    results.items.push({
      id: 'road',
      name: '道路上空',
      status: 'warning',
...
const generateRequiredProcedures = (results) => {
  ...
  if (
    didItem?.status === 'warning' ||
    airportItem?.status === 'error' ||
    altitudeItem?.status === 'error'
  ) {
    procedures.push({
      id: 'dips',
  ...
  const yellowZoneItem = prohibitionLaw.items.find(
    (i) => i.id === 'yellow_zone'
  )
  ...
  // 4. 土地管理者への確認
  const landItems = landManager.items.filter(
    (i) => i.status === 'warning' && i.id !== 'private_land'
  )
...
export const generateExternalLinks = (results) => {
  ...
  // 空港情報がある場合
  const airportItem = results?.categories?.[0]?.items?.find(
    (i) => i.id === 'airport'
  )
