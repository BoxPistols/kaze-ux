  ...
  for (const feature of geojson.features) {
    if (turf.booleanPointInPolygon(point, feature)) {
      ...
      if (import.meta.env.DEV) {
        console.log(
          `[DID] DETECTED: lat=${lat.toFixed(6)}, lng=${lng.toFixed(6)} -> ${areaName}`
        )
      }
...
export const checkDIDArea = async (lat, lng) => {
  try {
    ...
    if (prefectures.length === 0) {
      if (import.meta.env.DEV) {
        console.log(
          `[DID] No prefecture for: lat=${lat.toFixed(6)}, lng=${lng.toFixed(6)}`
        )
      }
    ...
    if (import.meta.env.DEV) {
      ...
      console.log(
        `[DID] Checking: lat=${lat.toFixed(6)}, lng=${lng.toFixed(6)} in [${prefNames}]`
      )
    }
    ...
    for (const prefecture of prefectures) {
      ...
      if (geojson) {
        ...
        if (result) {
          ...
          if (import.meta.env.DEV) {
            console.log(
              `[DID] ✓ DID検出: ${result.area} (${prefecture.nameJa})`
            )
          }
          return result
        }
    ...
    if (import.meta.env.DEV) {
      console.log(
        `[DID] ✗ DID外: lat=${lat.toFixed(6)}, lng=${lng.toFixed(6)} (checked ${prefectures.length} prefecture(s))`
      )
    }
    return {
      isDID: false,
...
export const debugCheckDID = async (lat, lng) => {
  ...
  if (prefecture) {
    ...
    if (cached) {
      const geojson = didPrefectureCache.get(cacheKey)
      console.log(
        `[DID Debug] GeoJSON features: ${geojson?.features?.length || 0}`
      )
    }
  }
...
export const preloadDIDDataForCoordinates = async (coordinates) => {
  ...
  if (loadedCodes.size > 0) {
    console.log(
      `[DID] Preloaded ${loadedCodes.size} prefecture(s):`,
      Array.from(loadedCodes).join(', ')
    )
  }
