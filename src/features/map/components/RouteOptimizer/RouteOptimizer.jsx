...
const RouteOptimizer = ({ isOpen, onClose, waypoints, onApplyRoute }) => {
  ...
  const handleOptimize = useCallback(async () => {
    ...
    try {
      const homePoint =
        homePointMode === 'custom' && customHomePoint ? customHomePoint : null
  ...
  const selectedDroneSpecs = selectedDroneId
    ? getFormattedSpecs(selectedDroneId)
    : null
    ...
    <div className='route-optimizer'>
      ...
      <div className='route-optimizer__modal'>
        ...
        <div className='route-optimizer__steps'>
          <div
            className={`route-optimizer__step ${step >= 1 ? 'active' : ''} ${step > 1 ? 'completed' : ''}`}>
          ...
          <div
            className={`route-optimizer__step ${step >= 2 ? 'active' : ''} ${step > 2 ? 'completed' : ''}`}>
            <span className='route-optimizer__step-number'>2</span>
            <span className='route-optimizer__step-label'>ドローン</span>
          </div>
          <div className='route-optimizer__step-connector' />
          <div
            className={`route-optimizer__step ${step >= 3 ? 'active' : ''} ${step > 3 ? 'completed' : ''}`}>
                ...
                {USE_CASES.map((useCase) => (
                  <div
                    ...
                    <div className='route-optimizer__usecase-info'>
                      <div className='route-optimizer__usecase-name'>
                        {useCase.name}
                      </div>
                      <div className='route-optimizer__usecase-desc'>
                        {useCase.description}
                      </div>
                    </div>
                    {selectedUseCase === useCase.id && (
                      <CheckCircle
                        className='route-optimizer__usecase-check'
                        size={20}
                      />
                    )}
                  </div>
              ...
              <div className='route-optimizer__drone-grid'>
                {drones.map((drone) => (
                  <div
                    ...
                    <div className='route-optimizer__drone-icon'>
                      {getIcon(drone.icon, 20)}
                    </div>
                    <div className='route-optimizer__drone-info'>
                      <div className='route-optimizer__drone-model'>
                        {drone.model}
                      </div>
                      <div className='route-optimizer__drone-specs'>
                        <span>
                          <Clock size={12} /> {drone.maxFlightTime}分
                        </span>
                        <span>
                          <Route size={12} />{' '}
                          {(drone.maxRange / 1000).toFixed(1)}km
                        </span>
                      </div>
                    </div>
                    {selectedDroneId === drone.id && (
                      <CheckCircle
                        className='route-optimizer__drone-check'
                        size={20}
                      />
                    )}
                  </div>
              ...
              {selectedDroneSpecs && (
                <div className='route-optimizer__drone-detail'>
                  ...
                  <div className='route-optimizer__drone-detail-grid'>
                    <div>
                      <Clock size={14} /> 飛行時間:{' '}
                      {selectedDroneSpecs.flightTime}
                    </div>
                    <div>
                      <Battery size={14} /> 有効時間:{' '}
                      {selectedDroneSpecs.effectiveTime}
                    </div>
                    <div>
                      <Route size={14} /> 航続距離:{' '}
                      {selectedDroneSpecs.maxRange}
                    </div>
                    <div>
                      <Plane size={14} /> 巡航速度:{' '}
                      {selectedDroneSpecs.cruiseSpeed}
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className='route-optimizer__step-content'>
              ...
              <div className='route-optimizer__option-group'>
                <label>
                  <input
                    ...
                    onChange={(e) =>
                      setOptions({ ...options, autoSplit: e.target.checked })
                    }
                  />
                  バッテリー自動分割
                </label>
                ...
                <label>
                  <input
                    ...
                    onChange={(e) =>
                      setOptions({
                        ...options,
                        checkRegulations: e.target.checked,
                      })
                    }
                  />
                  規制区域チェック
              ...
              <div className='route-optimizer__option-group'>
                ...
                <div className='route-optimizer__slider-group'>
                  ...
                  />
                  <span className='route-optimizer__slider-value'>
                    {Math.round(options.safetyMargin * 100)}%
                  </span>
                </div>
                <p className='route-optimizer__option-desc'>
            ...
            <div className='route-optimizer__step-content'>
              ...
              <div className='route-optimizer__summary'>
                <div className='route-optimizer__summary-item'>
                  ...
                  <div>
                    ...
                    <span className='route-optimizer__summary-label'>
                      総距離
                    </span>
                  </div>
                </div>
              ...
              {optimizationResult.summary.improvement > 0 && (
                <div className='route-optimizer__improvement'>
                  <CheckCircle size={16} />
                  ルート距離を{optimizationResult.summary.improvement}
                  %短縮しました
                </div>
              )}
              ...
              <div className='route-optimizer__home-info'>
                <Home size={16} />
                <span>
                  ホームポイント: {optimizationResult.homePoint.lat.toFixed(6)},{' '}
                  {optimizationResult.homePoint.lng.toFixed(6)}
                </span>
              ...
              <div className='route-optimizer__flights'>
                {optimizationResult.flights.map((flight) => (
                  <div
                    ...
                    className='route-optimizer__flight'>
                    <div
                      ...
                      <div className='route-optimizer__flight-detail'>
                        <div className='route-optimizer__flight-route'>
                          ...
                          {flight.waypoints.map((wp, idx) => (
                            <span
                              key={wp.id}
                              className='route-optimizer__flight-wp'>
                        ...
                        {flight.restrictions.length > 0 && (
                          <div className='route-optimizer__flight-warnings'>
                            {flight.restrictions.map((r, idx) => (
                              <div
                                key={idx}
                                className='route-optimizer__flight-warning'>
              ...
              {optimizationResult.restrictions.length > 0 && (
                <div className='route-optimizer__warnings'>
                  <AlertTriangle size={16} />
                  <span>
                    {optimizationResult.restrictions.length}
                    件の規制区域警告があります
                  </span>
                </div>
              )}
