...
const Map = ({
  ...
  const [selectedWaypointIds, setSelectedWaypointIds] = useState<Set<string>>(
    new Set()
  )
  ...
  const [rainCloudSource, setRainCloudSource] =
    useState<RasterSourceConfig | null>(null)
  const [windSource, setWindSource] = useState<RasterSourceConfig | null>(null)
  const [mapStyleId, setMapStyleId] = useState<BaseMapKey>(
  ...
  useEffect(() => {
    if (layerVisibility.showWind) {
      ...
      if (apiKey) {
        ...
        setWindSource(null)
        console.warn(
          'OpenWeather APIキーが設定されていません。（VITE_OPENWEATHER_API_KEY）'
        )
      }
    } else {
      setWindSource(null)
  ...
  const handleWaypointDoubleClick = useCallback(
    (e: React.MouseEvent, wp: Waypoint) => {
      ...
      if (
        onWaypointDelete &&
        confirm(`Waypoint #${wp.index} を削除しますか?`)
      ) {
  ...
  const handleSelectionStart = useCallback(
    (e: MapLayerMouseEvent) => {
      ...
      setIsSelecting(true)
      const canvas = (
        e.target as unknown as { getCanvas: () => HTMLCanvasElement }
      ).getCanvas()
  ...
  return (
    <div className={styles.mapContainer}>
      <MapGL
        ...
        {customLayers.map(
          (layer) =>
            visibleCustomLayerIds.has(layer.id) && (
              <Source
                key={layer.id}
                id={layer.id}
                type='geojson'
        ...
        {waypoints.map((wp) => {
          ...
          const flags: WaypointIssueFlags | null =
      ...
      {/* Map control buttons */}
      <div
        className={`${styles.mapControls} ${isMobile ? styles.mobileControls : ''} ${mobileControlsExpanded ? styles.expanded : ''}`}>
