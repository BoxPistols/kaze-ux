    ...
    useImperativeHandle(ref, () => ({
      ...
      finishEditing: () => {
        if (drawRef.current && editingIdRef.current) {
          ...
          const editedFeature = features.features.find(
            (f) => f.id === editingIdRef.current
          )
          if (editedFeature) {
            const edited: EditedPolygon = {
              id: editingIdRef.current,
              geometry: editedFeature.geometry as
                | GeoJSON.Polygon
    ...
    useEffect(() => {
      ...
      const handleUpdate = (e: DrawEvent) => {
        ...
        if (editingIdRef.current && e.features.length > 0) {
          const editedFeature = e.features.find(
            (f) => f.id === editingIdRef.current
          )
          if (editedFeature) {
            const edited: EditedPolygon = {
