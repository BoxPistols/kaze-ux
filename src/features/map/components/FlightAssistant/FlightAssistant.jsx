...
function FlightAssistant({
  ...
  const [proposedPlan, _setProposedPlan] = useState({
    altitude: 50,
    purpose: '点検飛行',
  })
  ...
  const [avoidanceDistance, setAvoidanceDistance] = useState(
    getSetting('didAvoidanceDistance') || 100
  )
  const [didAvoidanceMode, setDidAvoidanceMode] = useState(
    getSetting('didAvoidanceMode') ?? false
  )
  const [didWarningOnly, setDidWarningOnly] = useState(
    getSetting('didWarningOnlyMode') ?? false
  )
  ...
  const handleSaveChatLog = () => {
    const name = prompt(
      'チャットログの名前を入力してください:',
      `判定ログ ${new Date().toLocaleString('ja-JP')}`
    )
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (hasKey) {
        const response = await getFlightAdvice(userMessage, {
          polygons,
          waypoints,
        })
        setMessages((prev) => [
  ...
  const handleApplyOptimization = async () => {
    ...
    // 除外されたWPは元の位置を保持
    const filteredWaypoints = optimizationPlan.recommendedWaypoints.map(
      (wp) => {
  ...
  const handleAssessment = async () => {
    if (polygons.length === 0) {
      setMessages((prev) => [
        ...prev,
        {
          role: 'assistant',
          content:
            '[!] 判定を行うには、まず地図上でエリア（ポリゴン）を設定してください。',
        },
      ])
      return
    }
    ...
    try {
      ...
      // 機体推奨
      if (
        result.aircraftRecommendations &&
        result.aircraftRecommendations.length > 0
      ) {
        response += `### 推奨機体\n`
      ...
      if (applicationCosts?.applications?.length > 0) {
        ...
        if (
          applicationCosts.requiredDocuments &&
          applicationCosts.requiredDocuments.length > 0
        ) {
      ...
      console.log(
        '[FlightAssistant] DID waypointDetails:',
        result.context?.didInfo?.waypointDetails
      )
      const optimization = generateOptimizationPlan(
        polygons,
        waypoints,
        result.context?.didInfo
      )
      ...
      if (optimization.hasIssues) {
        ...
        if (optimization.waypointAnalysis.gaps.length > 0) {
          ...
          optimization.waypointAnalysis.gaps.forEach((gap) => {
            ...
            if (issue.type === 'airport') {
              ...
              actionText = gap.moveDistance
                ? `${Math.round(gap.moveDistance)}m移動`
                : '要移動'
        ...
        const hasNonDIDIssues = optimization.waypointAnalysis.gaps.some((g) =>
          g.issues.some((i) => i.type !== 'did')
        )
        if (hasNonDIDIssues) {
      ...
      setMessages((prev) => {
        ...
        return [
          ...filtered,
          { role: 'assistant', content: response, isAssessment: true },
        ]
      })
    } catch (error) {
  ...
  const generateAssessmentText = () => {
    ...
    // 必要な許可
    if (
      assessmentResult.requiredPermissions &&
      assessmentResult.requiredPermissions.length > 0
    ) {
      content += `## 必要な許可\n\n`
    ...
    if (
      ...
      optimizationPlan?.waypointAnalysis?.gaps?.length > 0
    ) {
      ...
      optimizationPlan.waypointAnalysis.gaps.forEach((gap) => {
        ...
        const recommendedPos = gap.recommended
          ...
          : '-'
        const moveDistance = gap.moveDistance
          ? `${Math.round(gap.moveDistance)}m`
          : '-'
      ...
      case 'LOW':
        return (
          <span className='risk-badge low'>
            <CheckCircle size={12} /> 低リスク
          </span>
        )
      case 'MEDIUM':
        return (
          <span className='risk-badge medium'>
            <AlertTriangle size={12} /> 中リスク
          </span>
        )
      case 'HIGH':
        return (
          <span className='risk-badge high'>
            <AlertTriangle size={12} /> 高リスク
          </span>
        )
      case 'CRITICAL':
        return (
  ...
  const panelStyle =
    isMobile || isFullscreen || isExpanded
      ? {}
      : {
    ...
    <div
      ...
      <div className='flight-assistant-header'>
        ...
        <div className='header-title'>
          ...
          {isPreConfigured ? (
            <span className='ai-badge preset' title='OpenAI APIキー標準搭載'>
              AI標準
            </span>
          ) : hasKey ? (
      ...
      {showChatLogs && (
        <div className='chat-logs-panel'>
          <div className='chat-logs-header'>
            <h3>
              <FolderOpen size={16} /> 保存済みログ
            </h3>
            <button
              className='close-btn'
              onClick={() => setShowChatLogs(false)}>
              <X size={14} />
            </button>
          </div>
            ...
            {chatLogs.length === 0 ? (
              ...
              chatLogs.map((log) => (
                <div
                  ...
                  <div
                    className='log-info'
                    onClick={() => handleLoadChatLog(log.id)}>
                    {editingLogId === log.id ? (
                      <input
                        type='text'
      ...
      <div className='flight-assistant-actions'>
        ...
        <div className='action-info'>
          <MapPin size={12} />
          <span>
            {polygons.length}エリア/{waypoints.length}WP
          </span>
        </div>
      </div>
            ...
            <div className='summary-detail'>
              {assessmentResult.context?.nearestAirport && (
                <div className='detail-row'>
                  <Plane size={12} />
                  <span>
                    最寄空港: {assessmentResult.context.nearestAirport.name}
                  </span>
                </div>
              )}
              <div className='detail-row'>
                <FileText size={12} />
                <span>
                  承認目安: {assessmentResult.estimatedApprovalDays}日
                </span>
              </div>
          ...
          <div className='recommendation-list'>
            <div className='recommendation-header'>
              ...
              <span className='recommendation-count'>
                適用:{' '}
                {
                  optimizationPlan.waypointAnalysis.gaps.filter(
                    (g) =>
                      g.recommended &&
                      !excludedRecommendations.has(g.waypointId)
                  ).length
                }
            ...
            <div className='recommendation-items'>
              {optimizationPlan.waypointAnalysis.gaps.map((gap) => {
                ...
                const isExcluded =
                  !hasRecommendation ||
                  excludedRecommendations.has(gap.waypointId)
                const issue = gap.issues[0]
                const zoneLabel =
                  issue?.type === 'airport'
                    ? '空港'
                    : issue?.type === 'prohibited'
                      ? '禁止'
                      : issue?.type === 'did'
                        ? 'DID'
                        : '警告'
                return (
                  <div
                    ...
                    onClick={() =>
                      hasRecommendation && toggleRecommendation(gap.waypointId)
                    }
                    title={
                      hasRecommendation
                        ? 'クリックで適用/除外を切替'
