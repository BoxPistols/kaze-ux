...
const ExportPanel = ({ waypoints = [], polygons = [], onClose }) => {
  ...
  const getPreviewTitle = () => {
    switch (previewMode) {
      case 'waypoint-json':
        return 'Waypoint JSON プレビュー'
      case 'waypoint-csv':
        return 'Waypoint CSV プレビュー'
      case 'waypoint-dms':
        return 'DMSテキスト プレビュー（度分秒）'
      case 'waypoint-dms-csv':
        return 'DMS CSV プレビュー（度分秒）'
      case 'polygon-geojson':
        return 'ポリゴン GeoJSON プレビュー'
      case 'polygon-kml':
        return 'ポリゴン KML プレビュー'
      case 'backup':
        return 'バックアップ プレビュー'
      default:
        return ''
  ...
  const renderPreview = () => {
    if (
      (previewMode === 'waypoint-csv' || previewMode === 'waypoint-dms-csv') &&
      previewData
    ) {
                ...
                <div key={polygon.id} className={styles.altitudeRow}>
                  ...
                  <div className={styles.altitudeInputWrapper}>
                    <input
                      ...
                      onChange={(e) =>
                        handleAltitudeChange(polygon.id, e.target.value)
                      }
        ...
        <div className={styles.previewActions}>
          <button
            ...
            onClick={() => {
              switch (previewMode) {
                case 'waypoint-json':
                  handleExportWaypointsJSON()
                  break
                case 'waypoint-csv':
                  handleExportWaypointsCSV()
                  break
                case 'waypoint-dms':
                  handleExportDMS()
                  break
                case 'waypoint-dms-csv':
                  handleExportWaypointsDMSCSV()
                  break
                case 'polygon-geojson':
                  handleExportPolygonsGeoJSON()
                  break
                case 'polygon-kml':
                  handleExportPolygonsKML()
                  break
                case 'backup':
                  handleExportBackup()
                  break
