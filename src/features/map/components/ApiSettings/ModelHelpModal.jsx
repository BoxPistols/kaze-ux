...
function ModelHelpModal({ onClose }) {
  ...
  const handleOverlayClick = (event) => {
    if (
      modalRef.current &&
      !modalRef.current.contains(/** @type {Node} */ (event.target))
    ) {
      onClose()
    }
  }
    ...
    <div className='model-help-overlay' onMouseDown={handleOverlayClick}>
      <div
        ...
        <div className='model-help-content'>
          <p className='lead'>
            迷ったら <strong>GPT-5 Nano</strong>
            。長文・大量データ（超長いプロンプト/ログ）を扱うなら{' '}
            <strong>GPT-4.1</strong> 系が向きます。
          </p>
          ...
          <div className='note'>
            ...
            <ul>
              <li>
                ここでの「速い/安い」は相対的な目安です（用途・負荷で変動します）。
              </li>
              <li>
                本アプリは OpenAI を使わない場合、<strong>ローカルLLM</strong>{' '}
                も選べます（互換性は環境次第）。
              </li>
            </ul>
          </div>
