...
function ApiSettings({ isOpen, onClose, onApiStatusChange }) {
  ...
  const [waypointNumbering, setWaypointNumbering] = useState(
    getWaypointNumberingMode()
  )
              ...
              <div className='connection-test'>
                ...
                {testStatus && testStatus !== 'testing' && (
                  <div
                    ...
                    {testStatus.success ? (
                      <>
                        <CheckCircle size={14} /> {testStatus.message} (
                        {testStatus.model})
                      </>
                    ) : (
                      <>
          ...
          <div className='settings-section'>
            ...
            <div className='zone-list'>
              ...
              <div className='zone-item'>
                ...
                <span
                  className={`zone-status ${didAvoidanceMode ? 'avoidance' : didWarningOnly ? 'warning' : 'off'}`}>
                  {didAvoidanceMode
                    ? '回避推奨'
                    : didWarningOnly
                      ? '警告のみ'
            ...
            <button
              ...
              onClick={async () => {
                const confirmed = await showConfirm({
                  title: '設定リセット',
                  message:
                    '判定設定をデフォルトに戻しますか？（APIキーは削除されません）',
                  confirmText: 'リセット',
                  cancelText: 'キャンセル',
                  variant: 'warning',
                })
