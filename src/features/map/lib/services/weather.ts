...
export async function getRainViewerSourceConfig(): Promise<RasterSourceConfig> {
  ...
  return {
    type: 'raster',
    tiles: [`${host}${latestRadar.path}/256/{z}/{x}/{y}/2/1_1.png`],
    tileSize: 256,
    minzoom: 1,
    maxzoom: 12,
 ...
 * 注意: APIキーが必要
 */
export function getWindLayerSourceConfig(
...
export type WeatherOverlayId =
  (typeof WEATHER_OVERLAY_IDS)[keyof typeof WEATHER_OVERLAY_IDS]
