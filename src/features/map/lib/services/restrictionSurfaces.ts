...
export type RestrictionSurfaceFeature = GeoJSON.Feature<
  GeoJSON.Geometry,
  RestrictionSurfaceProperties
>
...
export const KOKUAREA_TILE_URL =
  'https://maps.gsi.go.jp/xyz/kokuarea/{z}/{x}/{y}.geojson'
/** kokuarea は z=8 のみ提供 */
...
const storeTileCache = (
  key: string,
  features: RestrictionSurfaceFeature[]
): void => {
  ...
  const request = (async (): Promise<RestrictionSurfaceFeature[]> => {
    try {
      ...
      return geojson.features.map((feature) =>
        enrichRestrictionSurfaceFeature(feature)
      )
    } catch {
      return []
    }
  ...
  try {
    ...
    for (const feature of features) {
      ...
      if (
        ...
        const isInside = isPointInFeature(
    ...
    console.warn(
      '[RestrictionSurface] checkPointInRestrictionSurface error:',
      error
    )
