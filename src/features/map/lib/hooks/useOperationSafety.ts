...
export function useOperationSafety(
  ...
  const safetyEvaluation = useMemo(() => {
    ...
    if (!flightWindow.flightAllowedNow) {
      ...
      flightWindow.minutesRemaining < 30 &&
      flightWindow.minutesRemaining > 0
    ) {
      if (safetyLevel === 'safe') safetyLevel = 'caution'
      reasons.push({
        category: 'daylight',
    ...
    if (!canFly) {
      if (!network.hasLTE) {
        ...
        const safeForecast = weather.data.forecasts.find((f) => {
          const isWeatherSafe =
            f.windSpeed < 10 && f.precipitationProbability <= 50
    ...
    weatherData: weather.data?.forecasts[0]
      ? {
          ...
          precipitationProbability:
            weather.data.forecasts[0].precipitationProbability,
