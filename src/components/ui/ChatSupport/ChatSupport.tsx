...
const SYSTEM_PROMPT = `あなたは SDPF UI Theme コンシェルジュ。回答は必ず以下のルールに従うこと:
...
- **構築戦略**: 汎用ライブラリ(MUI)→Headless(Radix)→フルスクラッチ。SDPFはMUIベースハイブリッド
...
- **アセット**: アイコン=SVG+currentColor(テーマ自動追従)。SDPF=lucide-react
...
const normalizeShortcutBinding = (
  ...
  const key = typeof value.key === 'string' ? normalizeShortcutKey(value.key) : ''
...
const normalizeShortcuts = (value: unknown): ShortcutMap => {
  ...
  return {
    sendMessage: normalizeShortcutBinding(value.sendMessage, defaults.sendMessage),
    ...
    toggleUiMode: normalizeShortcutBinding(value.toggleUiMode, defaults.toggleUiMode),
    clearHistory: normalizeShortcutBinding(value.clearHistory, defaults.clearHistory),
...
const normalizeChatConfig = (value: unknown): ChatSupportConfig => {
  ...
  return {
    apiKey:
      typeof value.apiKey === 'string' ? value.apiKey : DEFAULT_CHAT_CONFIG.apiKey,
    model: typeof value.model === 'string' ? value.model : DEFAULT_CHAT_CONFIG.model,
...
const loadChatConfig = (): ChatSupportConfig => {
  ...
  if (!saved) return { ...DEFAULT_CHAT_CONFIG, shortcuts: createDefaultShortcuts() }
  try {
...
**SDPF推奨**: ボトムアップ。Design Tokensから始めて、小さなコンポーネントを組み合わせる
...
**SDPF**: MUIベースのハイブリッド戦略（汎用=MUI、ドメイン=独自）
...
**SDPF方針**: lucide-react + MUI Icons を標準使用
...
Googleが提唱する設計体系。SDPFは **MUI（Material UI）v7** を通じてこの体系をベースにしている。
...
### 重要概念
- **Elevation（影の深度）**: 0=フラット、1-24=浮き上がり。SDPFはCard=elevation 0（フラット）を基本
- **Ripple（波紋）**: タッチ/クリック時の波紋エフェクト。MUIボタン標準搭載
- **8pxグリッド**: 全スペーシングを8の倍数で統一。SDPFも同様
...
const INITIAL_GREETING =
  'SDPF コンシェルジュです。トークン・コンポーネント・設計理論について質問できます。下のボタンか自由入力でどうぞ。'
        ...
        <Stack direction='row' spacing={1.5} alignItems='center'>
          ...
          <Box>
            <Typography variant='subtitle2' sx={{ fontWeight: 600 }}>
              {showSettings ? 'AI設定' : 'SDPF Concierge'}
            </Typography>
            {!showSettings && (
              <Typography
                variant='caption'
              ...
              <Box
                ...
                {storyGuide && (
                  <Chip
                    ...
                    size='small'
                    variant='outlined'
                    onClick={() => handleSuggestionClick('この画面は何ですか？')}
                    sx={{
                      cursor: 'pointer',
                      fontSize: 12,
