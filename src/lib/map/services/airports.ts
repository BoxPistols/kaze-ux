 ...
 *
 * 制限半径の根拠:
 * - 国際空港・主要空港: 小型無人機等飛行禁止法対象8空港は約24km
...
export function generateAirportGeoJSON(): GeoJSON.FeatureCollection {
  const features: GeoJSON.Feature[] = getAllAirports().map((airport) => ({
    ...
    geometry: createCirclePolygon(airport.coordinates, airport.radiusKm),
  }))
...
> {
  const features: Array<
    GeoJSON.Feature<GeoJSON.Point, AirportMarkerProperties>
