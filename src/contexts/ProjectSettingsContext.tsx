...
/**
 * プロジェクト設定コンテキスト
 * アプリケーション全体でプロジェクト設定を共有するためのコンテキスト
 */
...
export const ProjectSettingsProvider = ({
  ...
  const [settings, setSettings] = useState<ProjectSettings>(() => ({
    ...
    ...initialSettings,
    units: {
      ...DEFAULT_PROJECT_SETTINGS.units,
      ...initialSettings?.units,
    },
    dateTime: {
      ...DEFAULT_PROJECT_SETTINGS.dateTime,
      ...initialSettings?.dateTime,
    },
    mockData: {
      ...DEFAULT_PROJECT_SETTINGS.mockData,
  ...
  const applyPreset = useCallback((preset: ProjectPreset) => {
    const presetSettings = PROJECT_PRESETS[preset]
    setSettings((prev) => ({
      ...prev,
      ...presetSettings,
    }))
...
export const useProjectSettingsOptional = (): ProjectSettingsContextValue => {
  const context = useContext(ProjectSettingsContext)
  if (!context) {
    // デフォルト設定を使用
    const defaultFormatters = {
      units: createUnitFormatter(DEFAULT_PROJECT_SETTINGS.units),
      date: (date: Date) =>
        formatDateWithSettings(date, DEFAULT_PROJECT_SETTINGS.dateTime),
      time: (date: Date) =>
        formatTimeWithSettings(date, DEFAULT_PROJECT_SETTINGS.dateTime),
      dateTime: (date: Date) =>
        formatDateTimeWithSettings(date, DEFAULT_PROJECT_SETTINGS.dateTime),
      relativeTime: (date: Date) =>
        formatRelativeTimeWithSettings(date, DEFAULT_PROJECT_SETTINGS.dateTime),
    }
