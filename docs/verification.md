...
flowchart TB
  ...
  subgraph L2["層 2 — 実描画の計測（ソースからは見えないものを捕まえる）"]
    ...
    C2["check:a11y<br/>138 story × 2 テーマ<br/>コントラスト + 操作対象寸法"]
...
| dark-dracula                         | `textContrast` が `X.lighter` の面に対して未保証（#121）。`A11Y_THEMES` で測れる |
...
- **dark-dracula は未対応**（#121）
- **アプリ 3 つの実画面**は `audit:contrast` で測れるが、CI には載っていない（起動が要るため）
