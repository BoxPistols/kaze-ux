...
**これは移行作業を行った当時の記録であり、kaze-ux の現行仕様書ではない。**
以降の本文に出てくるモデル名・既定値・アクセス制御は当時のもので、現在の実装とは一致しない。
手順とハマりどころ（パラメータ差分、SDK ルーティング、レート制限の設計、失敗パターン）は
今も有効なので、そこを参照する目的で残している。

### kaze-ux の現在の構成

単一ソースは `src/components/ui/ChatSupport/chatSupportConstants.ts`。
